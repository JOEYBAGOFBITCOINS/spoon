...
function init() {
	...
	document.title = "Self-Reflection";
...
class Renderer {
	...
	createCubeMap() {
		...
		const faces = [
			...
			[gl.TEXTURE_CUBE_MAP_POSITIVE_Y, "02posy.jpg"],
			[gl.TEXTURE_CUBE_MAP_NEGATIVE_Y, "02negy.jpg"],
			[gl.TEXTURE_CUBE_MAP_POSITIVE_Z, "02posz.jpg"],
			[gl.TEXTURE_CUBE_MAP_NEGATIVE_Z, "02negz.jpg"]
		];
	...
	init() {
		...
		program.wood = gl.getUniformLocation(program, "wood");
	}
	render(now = 0) {
		const {
			gl,
